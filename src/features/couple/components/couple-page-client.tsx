      ...
      <AuthGuard>
        <div className="p-4 space-y-4">
          <div className="text-center py-8">
            ...
            <p className="text-rhythm-text-secondary text-sm">还没有绑定情侣关系</p>
          </div>
          ...
          {showInviteInput && (
            <div className="r-card p-4 space-y-3">
              <input
                ...
                maxLength={6}
                className="r-input text-center tracking-widest uppercase"
              />
    ...
    return (
      <AuthGuard>
        <div className="p-4 space-y-4">
          <div className="text-center py-8">
            <p className="text-4xl mb-3">💌</p>
            <p className="text-rhythm-text-secondary text-sm mb-3">邀请已创建，告诉对方你的邀请码</p>
            <div className="text-3xl font-bold tracking-[0.5em] text-rhythm-glow bg-rhythm-glow-soft border border-rhythm-border rounded-xl py-4 px-8 inline-block">
              {code}
            </div>
            <p className="text-xs text-rhythm-text-muted mt-3">邀请码 48 小时内有效</p>
          </div>
          <button
            type="button"
            onClick={cancelInvite}
            className="w-full py-2.5 border border-rhythm-danger/40 text-sm text-rhythm-danger rounded-xl hover:bg-rhythm-danger-soft transition-colors"
          >
            取消邀请
          </button>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="r-card p-4 bg-rhythm-glow-soft">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-rhythm-void/40 border border-rhythm-border rounded-full flex items-center justify-center text-lg text-rhythm-text-primary">
              {profile?.nickname?.[0] ?? '我'}
            </div>
            <span className="text-2xl text-rhythm-danger">❤</span>
            <div className="w-12 h-12 bg-rhythm-void/40 border border-rhythm-border rounded-full flex items-center justify-center text-lg text-rhythm-text-primary">
              {partner?.nickname?.[0] ?? '?'}
            </div>
          </div>
          <p className="text-sm text-rhythm-text-secondary mt-2">
        ...
        <div className="r-card p-4">
          <h3 className="r-title text-sm mb-3">说点什么</h3>
            ...
            {([
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs transition-colors ${
                  encourageType === type ? 'bg-rhythm-glow-soft text-rhythm-glow border border-rhythm-glow' : 'bg-rhythm-void/40 border border-rhythm-border text-rhythm-text-muted hover:text-rhythm-text-secondary'
                }`}
              >
                {emoji} {type}
              </button>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="r-btn-primary px-4 py-2 text-sm disabled:opacity-50"
            >
              发送
            </button>
          </div>
        ...
        {/* Shared permissions */}
        <div className="r-card p-4">
          <h3 className="r-title text-sm mb-3">共享设置</h3>
          <p className="text-xs text-rhythm-text-muted mb-3">设置你愿意共享给 ta 的数据</p>
          ...
          {([
            ...
            return (
              <div key={dataType} className="flex items-center justify-between py-2 border-b border-rhythm-border last:border-0">
                <div className="flex items-center gap-2">
                  <span>{icon}</span>
                  <span className="text-sm text-rhythm-text-secondary">{label}</span>
                </div>
        ...
        {showDisbandConfirm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="r-card p-5 max-w-xs w-full">
              <h3 className="r-title mb-2">确认解除关系？</h3>
              <p className="text-sm text-rhythm-text-secondary mb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="r-btn-ghost flex-1 py-2 text-sm"
                >
                  取消
                </button>
                <button
                  ...
                  className="flex-1 py-2 text-sm text-rhythm-danger border border-rhythm-danger/40 bg-rhythm-danger-soft rounded-xl hover:opacity-80 transition-opacity"
                >
                  确认解除
                </button>
              </div>
