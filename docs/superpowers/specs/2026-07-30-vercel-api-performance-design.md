...
5. PostgreSQL Pool 空闲连接保留 60 秒，覆盖常见页面切换间隔，同时继续限制单实例最多 5 个连接。
6. 保持现有页面展示、认证 Cookie 和 API 返回字段兼容。
...
线上诊断确认鉴权热路径小于 1ms、业务查询约 220ms，而 `pool.connect()`
在连接被回收后需要 0.9–1.9 秒。`pg` 默认空闲 10 秒回收连接，不足以覆盖
正常页面切换，因此显式设置 `idleTimeoutMillis: 60_000`。不设置 `min`，
超过 60 秒后仍允许连接释放，避免每个 Vercel 实例永久占用 TencentDB 连接。
...
- 全量单元测试、类型检查和生产构建通过。
- 连接池配置测试确认空闲回收时间为 60 秒；部署后 60 秒内重复请求的 `db-connect` 应接近 0ms。
