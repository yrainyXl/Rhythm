    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="r-card p-3 text-center">
          <p className="text-xs text-rhythm-text-muted">本月运动</p>
          <p className="text-xl font-bold text-rhythm-text-primary">{analysis.totalSessions} 次</p>
        </div>
        <div className="r-card p-3 text-center">
          <p className="text-xs text-rhythm-text-muted">总时长</p>
          <p className="text-xl font-bold text-rhythm-text-primary">
      ...
      {analysis.categoryBreakdown.length > 0 && (
        <div className="r-card p-4">
          <h3 className="r-title text-sm mb-3">运动分布</h3>
          <div className="space-y-2">
            {analysis.categoryBreakdown.map(({ category, count, duration }) => (
              <div key={category} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-xs text-rhythm-text-muted w-16 text-right">
                  {count}次 / {duration}m
                </span>
              </div>
            ))}
      ...
      {analysis.weeklyTrend.length > 0 && (
        <div className="r-card p-4">
          <h3 className="r-title text-sm mb-3">每周趋势</h3>
          <div className="space-y-2">
            {analysis.weeklyTrend.map(({ week, sessions, duration }) => (
              <div key={week} className="flex items-center gap-3 text-sm">
                <span className="text-rhythm-text-secondary w-20 text-xs truncate">{week}</span>
                <div className="flex-1 bg-rhythm-void/40 rounded-full h-2">
                  <div
                    className="bg-rhythm-success rounded-full h-2"
                    style={{
      ...
      {analysis.rehabProgress.length > 0 && (
        <div className="r-card p-4 bg-rhythm-glow-soft border-rhythm-glow">
          <h3 className="r-title text-sm mb-2 text-rhythm-glow">康复进展</h3>
          {analysis.rehabProgress.map(({ templateName, sessions, daysSinceStart }) => (
            <div key={templateName} className="text-sm text-rhythm-text-secondary">
              <p>
